...
Since the US Census tries to ensure census tracts have populations between 2,000 and 4,000 individuals, regions that experience growth in population density require that tracts be split over time and regions that are losing population need to combine tracts. As a result, comparing tracts over time can be tricky since the geographies change. 

Harmonization of census data is the process of translating data from multiple decades onto a single geography so that changes over time are meaningful. This requires a weighted sampling process called apportionment. Typically you use the most recent map because it will have the most geographic units (you are more likely to split tracts over time than to combine them), and thus supports the most granular analysis. *Note that tracts are only redrawn each 10 years, so 2012 and 2017 data would come from the same geographies. Data from 2008 and 2012, however, would not (tracts were re-drawn in 2010).*

For this lab we will use data from the [Longitudinal Tract Database (LTDB) Project](https://s4.ad.brown.edu/Projects/Diversity/Researcher/Bridging.htm), a project that has harmonized a set of variables from 1970 to 2010. 
